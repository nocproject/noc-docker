  ...
  if [ ! -f "$INSTALLPATH"/data/noc/etc/noc.conf ]
    then
        ...
        { echo "NOC_CONFIG=env:///NOC"
          ...
          echo "# Cache method: mongo(noc.core.cache.mongo.MongoCache) or Radis(noc.core.cache.redis.RedisCache)"
          echo "NOC_CACHE_CACHE_CLASS=noc.core.cache.mongo.MongoCache"
