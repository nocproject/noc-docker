    ...
    def handle(
        ...
        **options
        ...
        for x in self.enable_ping:
            ...
            data += s + "\n"
            # file.write(s + "\n")
        fn = "/tmp/report%s.csv" % (input[0].replace("/", "-"))
        file = open(fn, "w")
        file.write(data)
        file.close()
        ...
        if email:
            ...
            for adr in self.nets:
                bodymessage += adr + " " + self.time[adr] + " сек.\n"
            filename = "found_ip_%s_%s" % (pool, datetime.datetime.now().strftime("%Y%m%d"))
            if format == "csv":
                f = "%s.csv" % filename
                ...
                attach = [
                    {
                        "filename": f,
                        "data": base64.b64encode(response.getvalue()),
